...
| 3 | ~~能按 turn 前进/后退~~；工具调用与结果**配对**展示（`toolCallId`，合成一条记录）**——2026-09-14 用户定：去掉"按 turn 前进/后退"**。理由：**已确认的原型里只有「只看错误 / 下一个错误」，从没有过轮次跳转控件**，补它是 AI 发明用户没要过的东西；轮次靠 `T1/T2` 刻度标记 + 滚动已经够用（v2 的密列表 + 泳道总览才是轮次导航的正解，见 C20） |
| 4 | 每条记录可见 **token / 耗时 / 输入 / 输出** |
| 5 | **打开 877KB 轨迹不卡**（2026-09-14 实测改为：**不是渲染问题**——最大那条 1149KB / 3222 entry 折叠后**只有 11 行**、`readTrace` 12ms。虚拟化不需要，见 §6） |
...
- **老 154 个 trace 文件归不到会话**（`trace_start` 里没有 sessionId，且**补不回来**）：不迁移、不删，读取时"文件名解析不出合法 sessionId 就跳过"。**不是选型变量**（A/B/C/D 四种方案下都一样）
  - **⚠️ 实测过"考古"可行性，并决定不做（2026-09-14 用户定）**：轨迹里没写会话 id，但有**用户原话**（`message_start` 与会话文件里存的是同一段文字），所以可以拿首条用户指令前 16 字去比对会话文件。实测 154 条：**可唯一归属 46 / 命中多个（歧义）22 / 找不到 86**（对应会话文件已删或被 `/clear` 清过）。**决定不做**，三个理由：① 收益低——这批轨迹对应的是**当时的代码状态**（8 月中 ~ 09-02），拿来调试当下的问题价值有限 ② **风险不对称**——22 条有歧义，**错误的归属比没有归属更糟**（沿用"缺失就不猜"的原则，同 `durationMs` 缺失时不显示耗时）③ 后患——推断出的文件混进同一套命名后**再也分不清哪些是真实记录、哪些是猜的**，除非再加 inferred 标记，那是给一次性动作引入永久复杂度。
  - **这个缺口会自然过期**：任何会话只要在新代码下再跑一次就有轨迹；老会话（8 月中 ~ 09-02）永远空。所以"有没有轨迹"取决于它最后一次跑发生在新代码之前还是之后，**不是永久性问题**。
...
- **⭐ 实测：轨迹文件大 ≠ 记录多（2026-09-14，影响虚拟化决策）**：`readTrace` 读 `.traces/` 最大的三条并折算记录数——
  | 文件 | 字节 | entries | **折叠后记录** | readTrace |
  |---|---|---|---|---|
  | 最大 | 1149KB | 3222 | **11** | 12ms |
  | 次大 | 857KB | 1217 | 22 | 6ms |
  | 第三 | 856KB | 1774 | 26 | 7ms |
  真实数据下记录数是**几十行**量级 → **虚拟滚动不需要**（离它的场景差两个数量级）。**验收第 5 条"打开 877KB 不卡"因此可判 ✅**——瓶颈不在渲染（11 行），在读文件 + 解析（12ms）。
- **⭐ 顺带发现：轨迹文件的体积被 `message_update` 的重复负载撑大（未处理）**：每条 `message_update` 都带**完整的累积消息对象**（不是只带增量），而**我们的折叠只用 `event.delta`**——`message_update.message` 在整个折叠链路里没有任何人读。于是 3266 条 entry 累积成 1.1MB，却只折叠出 11 条记录（长度 × 条数 ≈ 平方级）。
  - **同族病第 4 例**：`header.version` 从不被读 / `TraceSnapshot` 无人传 / `end()` 的 `error` 曾从不被传（A3 已接上）。区别是**这一例还吃磁盘与传输**。
  - **要处理得动 `lib/trace` 的记录层**（只在 update 里存增量，或干脆不存 `message`），属于 trace **格式**的事——**不塞进 A3 收尾**，记在这里待排。
