...
**判断架构要不要变的信号**（PLAN.md 第三节）：需要常驻后台的 agent 服务独立于 web、要发 CLI、要跨进程复用内核——到那时把 `lib/` 抽成共享包（monorepo），这是"换架构"的正确时机，不是现在。
...
> 第 1-9 节是"设计"，本节是"怎么让代码符合设计"。分两类任务：**合规任务**（补文档里缺的层——结构债）+ **功能任务**（在接缝上长功能，复用 PLAN.md 第十三节的施工单）。
