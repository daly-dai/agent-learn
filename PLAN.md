...
| B17 | **前端切会话竞态 + 渲染优化**（体检报告行动②⑥）：① `use-agent-run.ts` SSE abort + `applyFrame` 加 sessionId 校验（旧 run 帧不污染新会话，历史拉取已有 AbortController 照抄）② TaskPanel dismissed 加 `key={sessionId}` ③ Markdown/MessageRow memo + foldEvents useMemo（消每 delta 全树重渲染 + O(n²) 重折叠） | 体检报告前端专项 | `app/lib/use-agent-run.ts` + `app/components/{task-panel,message-row,markdown}.tsx` + `app/repos/page.tsx` | ⏳ 排队（验收：run 中切会话无旧帧混入；流式时不再全树重渲染） |
